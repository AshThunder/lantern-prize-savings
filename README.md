...
A [PoolTogether V5](https://dev.pooltogether.com/protocol/design/) prize vault on the [Zama Protocol](https://www.zama.org/post/zama-developer-program-mainnet-season-4): no-loss principal, encrypted deposit sizes, and **onchain FHE-weighted draws**. Winners are chosen at claim time with PoolTogether `isWinner` over encrypted TWAB — the winner address is never published.
...
Use **http://localhost:5173** or the live URL. Not `127.0.0.1` (Openfort rejects it).
...
7. After **60s** (or owner **Force + finish**), **Run draw**. That closes the round, snapshots encrypted TWAB, finishes, and checks this wallet. **Start only** closes the period and leaves Finish for a second tx.
...
| [Twab Controller](https://dev.pooltogether.com/protocol/design/twab-controller) | Running encrypted vault TWAB. `startDraw` freezes the total in one mul+add. User weight freezes at claim. `minHoldSeconds` zeros last-second deposits | [TWAB](https://dev.pooltogether.com/protocol/design/twab-controller) · [HCU](https://docs.zama.org/protocol/solidity-guides/development-guide/hcu) · [TWAB.md](./TWAB.md) |
...
| Prize tiers (grand + daily) | Two independent FHE tickets: **grand 75%** and **daily 25%** of prize liquidity | [Prize structure](https://dev.pooltogether.com/protocol/design/#prize-structure) |
...
PoolTogether V5 `isWinner`: `uniform(userSeed, vaultTwabTotal) < userTwab × tierOdds`. Lantern does that in FHE at **claim time**. The vault total is a running encrypted TWAB, frozen in `startDraw`. It does not walk the list to pick a winner. Design: [TWAB.md](./TWAB.md).

```solidity
uint64 seed = uint64(uint256(keccak256(abi.encodePacked(lastDrawEntropy, account, tier))));
euint64 ticket = mapTicket(seed, eligibleTwabTotal); // (seed * total) >> 64
ebool won = ticket < (weight * tierBps / 10_000);
pay = select(won, remainingTierPrize, 0); // at most one payout per tier
```

`userSeed` is public (draw entropy + address + tier). Weight and total stay encrypted. Grand 75% and daily 25% are independent. Encrypted remaining caps each tier at one prize. The winner address is never decrypted onchain.

Users learn the outcome by **Check prize** (`accruePrize`) then EIP-712 decrypt of `confidentialWinningsOf`, or by **Claim** (accrue + confidential transfer). Losers should claim too (encrypted 0) so a lone claim tx does not leak.

Zama [HCU](https://docs.zama.org/protocol/solidity-guides/development-guide/hcu) limits FHE ops **per transaction** (5M sequential depth, 20M global). Vault TWAB is updated as shares change so a draw does not add every depositor in one tx. See [TWAB.md](./TWAB.md).
...
| --- | --- |
| Individual deposits, vault shares, odds | Depositor addresses (needed to snapshot TWAB) |
| FHE tickets / `isWinner` | Wrap / unwrap ERC-20 amounts |
...
15 Hardhat tests cover deposit, sponsor split, claim-time `isWinner`, running TWAB, keeper pay. List: [contracts/README.md](./contracts/README.md).
