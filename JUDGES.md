...
A [PoolTogether V5](https://dev.pooltogether.com/protocol/design/) prize vault on the Zama Protocol. Deposit sizes stay encrypted. Draws run onchain with FHE. Winners are chosen at **claim time** with PoolTogether `isWinner` over encrypted TWAB. The winner address is never published. Principal always comes back.
...
8. After **60s** (or owner **Force + finish**), **Run draw**. Closes the round, snapshots encrypted TWAB, finishes, checks this wallet.
...
| [README.md](./README.md) | `isWinner` math, replica table, 13 tests |
| [SECURITY.md](./SECURITY.md) | Residual leak and what is public on purpose |

15 Hardhat tests: `cd contracts && npm ci && npx hardhat test`
